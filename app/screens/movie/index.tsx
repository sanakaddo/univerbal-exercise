...
import { getMovieByIdQuery, getMoviePosterUrlById } from '@/infrastructure/repositories/movie';
...
export default function MovieScreen({ route }: MovieScreenProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const movieData = await getMovieByIdQuery(controller.signal, route.params.id);
